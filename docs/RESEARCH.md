...
**가설**: 하나의 함수에 포함된 의미론적 개념의 수가 증가하면,
LLM이 해당 함수 내의 특정 결함을 정확히 식별하는 능력이 저하된다.

**교란 변인 분리**: 코드 길이(토큰 수)가 아니라 개념 수가 원인인지 확인하기 위해
3가지 버전을 비교.

| 버전 | 개념 수 | 코드 줄 수 | 설명 |
|------|--------|----------|------|
| V7 | 7 | 17줄 | 통제군: 주문 처리 기본 |
| V7+패딩 | 7 | 65줄 | 길이 통제: 무관한 함수 3개 추가 (개념은 동일) |
| V15 | 15 | 45줄 | 실험군: 쿠폰, 세금, 배송, 로열티, 알림, 감사로그 추가 |

**과제**: 모든 버전에 동일한 버그를 1개 심음 (off-by-one: `> 10` → `>= 10`).
"이 코드에 버그가 정확히 1개 있다. 찾아라."

**모델**: quick (경량), ultrabrain (고성능)

### 결과

| 버전 | 개념 | 줄 수 | quick | ultrabrain |
|------|------|------|-------|-----------|
| V7 | 7 | 17 | ✅ 정확 | ✅ 정확 |
| V7+패딩 | 7 | 65 | ✅ 정확 | ✅ 정확 |
| V15 | 15 | 45 | ❌ **오탐** | ✅ 정확 |

추가: 개념 수 9, 12에서도 quick 모델 오탐 확인 (1회차 실험).

| 버전 | 개념 | quick (1회차) |
|------|------|-------------|
| V9 | 9 | ❌ 오탐 (세금 순서를 버그로 오판) |
| V12 | 12 | ❌ 오탐 (세금 순서를 버그로 오판) |

### 교란 변인 분리

| 비교 | 통제한 것 | 결과 | 의미 |
|------|---------|------|------|
| V7 vs V7+패딩 | 개념 동일(7), 길이 17→65 | 둘 다 정확 | **코드 길이는 원인이 아님** |
| V7+패딩 vs V15 | 길이 유사(65 vs 45), 개념 7→15 | 패딩 정확, V15 오탐(quick) | **개념 수가 원인** |
| quick vs ultrabrain (V15) | 동일 코드, 모델만 다름 | quick 오탐, ultrabrain 정확 | **모델 추론 능력에 따라 임계값 다름** |

### 관찰된 것 (사실)

1. **코드 길이가 아니라 의미론적 개념의 수가 결함 탐지 정확도에 영향한다.**
   - V7+패딩(65줄, 개념 7) > V15(45줄, 개념 15): 더 긴 코드가 더 정확.

2. **개념 수가 증가하면 quick 모델은 실제 버그를 놓치고 존재하지 않는 버그를 만들어낸다(환각).**
   - V9, V12, V15 모두에서 "세금 계산 순서 버그"를 환각 — 실제로는 정상 로직.

3. **같은 개념 수 15에서 ultrabrain은 정확하고 quick은 오탐한다.**
   - 임계값이 모델의 추론 능력에 따라 다름.

### 관찰되지 않은 것 (주장하면 안 되는 것)

- ❌ 정확한 임계값 수치 (2개 모델, 반복 횟수 부족)
- ❌ "Miller's Law가 LLM에 적용된다" (구조적 유사성이지 동일 메커니즘 아님)
- ❌ 모든 LLM에 보편적 (2개 모델 카테고리만 테스트)
- ❌ 9가 최적 임계값 (quick에서 9부터 오탐이지만 n=1)

### 한계

- quick 모델 V9/V12는 1회만 실행 — 비결정성으로 인한 변동 가능
- 인공 코드(주문 처리) 사용 — 실제 오픈소스 코드에서 재현 필요
- 버그 유형이 off-by-one 1가지만 — 다른 버그 유형에서도 동일 패턴인지 미확인
- 주석에 `# BUG`가 포함되어 있어 힌트 제공 가능성 — 주석 제거 실험 필요
