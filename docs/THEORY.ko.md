...
| SA/SR/AR 조건부 경고 | Sutter & Alexandrescu 2004 [6], n=94 SURVEY | 2가지 조합은 파일 수준 90%에서 발견(보편적). 함수 수준에서 경계 조건(await 가로지름, 정리 부재, broad except) 충족 시에만 위험 | 경고 조건 충족 코드의 버그 발생률 비교 |
