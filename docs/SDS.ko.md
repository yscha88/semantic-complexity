...
시스템은 ML 파이프라인과 유사한 3단계 처리 구조를 따릅니다:
...
║  ┌─────────────────────────────────────────────────────────────┐  ║
║  │  5D Vector: [C, N, S, A, Λ]                                 │  ║
║  │  + Pattern Detection (Trust Boundary, Secret, Test 등)      │  ║
║  └─────────────────────────────────────────────────────────────┘  ║
╚═══════════════════════════════════════════════════════════════════╝
                                    │
                                    ▼
...
**핵심 원칙**: INPUT은 context-free (결정적), PROCESSING에서 context 주입, OUTPUT은 context-aware.
...
│  └─────────────────────────────────────────────────────────┘  │
├─────────────────────────────────────────────────────────────────┤
│                     Analysis Flow Layers                        │
├─────────────────────────────────────────────────────────────────┤
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐ ┌──────────┐ │
│  │ 정보 흐름   │ │ 인프라 흐름 │ │비즈니스흐름 │ │ 보안 흐름│ │
│  │  분석       │ │   분석      │ │   분석      │ │   분석   │ │
│  └─────────────┘ └─────────────┘ └─────────────┘ └──────────┘ │
├─────────────────────────────────────────────────────────────────┤
│                       Evaluation Levels                         │
...
```
🍞 Bread = 보안 신뢰 경계가 명확히 정의되어 있는가?

분석 대상:
┌─────────────────────────────────────────────────────────────┐
│ 요소              │ 탐지 방법              │ 심각도          │
├───────────────────┼────────────────────────┼─────────────────┤
│ Trust Boundary    │ 데코레이터, 마커       │ 필수            │
│ Auth Explicitness │ AUTH_FLOW 선언         │ 필수            │
│ Secret Pattern    │ 하드코딩 검사          │ high/medium/low │
│ Secret Leak       │ print/logger 출력      │ high            │
│ Blast Radius      │ import/LOC 분석        │ 참고용          │
└─────────────────────────────────────────────────────────────┘
```

#### 3.3.2 의사코드: Trust Boundary 탐지
...
```
🥓 Ham = Critical path가 Golden test로 보호되고 있는가?

분석 대상:
┌─────────────────────────────────────────────────────────────┐
│ 요소              │ 설명                   │ 보호 기준       │
├───────────────────┼────────────────────────┼─────────────────┤
│ Critical Path     │ payment, auth, data등  │ 100% 보호 권장  │
│ Golden Test       │ 예상 결과 assertion    │ PoC: 60%+       │
│ Contract Test     │ API 계약 검증          │ external API 시 │
│ Test Discovery    │ 자동 테스트 파일 탐색  │ test_*.py 등    │
└─────────────────────────────────────────────────────────────┘
```

#### 3.4.2 의사코드: Critical Path 탐지

```
FUNCTION detect_critical_paths(code) -> List[CriticalPath]:
    """
    Critical path 탐지 - 반드시 테스트로 보호해야 하는 경로
    """
...
#### 3.4.3 의사코드: Test Discovery

```
FUNCTION discover_tests(file_path: String, test_dir: String = None) -> Dict[String, String]:
    """
    소스 파일에 대응하는 테스트 파일 자동 탐색

    규칙:
    - src/module.py → tests/test_module.py
    - src/module.py → src/module_test.py
    - src/module.py → test/module_test.py
    """
...
#### 3.4.4 의사코드: Golden Test 탐지

```
FUNCTION detect_golden_tests(test_sources: Dict[String, String]) -> List[GoldenTest]:
    """
    Golden test 탐지 - 예상 결과를 assertion으로 검증하는 테스트
    """

    golden_tests = []

    GOLDEN_PATTERNS = [
        '@pytest.mark.golden',           # 명시적 golden marker
        'snapshot',                       # snapshot testing
        'assert.*==.*expected',          # expected 값 비교
...
FUNCTION calculate_ham_coverage(
    ...
    """
    Critical path 보호 커버리지 계산

    Returns:
        0.0 ~ 1.0 (보호된 critical path 비율)
    """

    IF len(critical_paths) == 0:
        RETURN 1.0  # Critical path 없으면 100%

    protected_count = 0

    FOR path IN critical_paths:
        # 테스트 파일에서 해당 함수를 호출하는지 확인
        FOR file_path, source IN test_sources.items():
...
```
Gate 시스템 = 코드 품질 진입 조건 (단계별 점진적 엄격화)
...
│ (실험 단계)  │               │             │               │
├──────────────┼───────────────┼─────────────┼───────────────┤
│ MVP          │ boundary정의  │ accessible  │ coverage≥80% │
│ (제품 단계)  │ AUTH_FLOW명시 │ no secrets  │ contract존재  │
├──────────────┼───────────────┼─────────────┼───────────────┤
│ Production   │ MVP 조건      │ MVP 조건    │ coverage≥95% │
│ (운영 단계)  │ + 감사 로그   │ + waiver 허용│ + 100% 권장  │
└─────────────────────────────────────────────────────────────┘
```
...
FUNCTION check_gate(
    ...
    Args:
        source: 분석 대상 소스 코드
        gate_type: Gate 단계
        file_path: 파일 경로 (test discovery용)
        test_dir: 테스트 디렉토리
    ...
    IF gate_type == "poc":
        # 🍞 Bread: SKIP
        # 🧀 Cheese: WARN only (violations에 추가하지만 실패는 아님)
        # 🥓 Ham: coverage >= 60%
    ...
    ELSE IF gate_type == "mvp":
        # 🍞 Bread: trust boundary 정의, AUTH_FLOW 명시
    ...
    ELSE IF gate_type == "production":
        ...
        # Waiver 확인 (Production만 허용)
        # Waiver는 별도 check_waiver 함수에서 처리
...
```
Essential Complexity Waiver = 본질적 복잡도에 대한 예외 허용

적용 조건:
┌─────────────────────────────────────────────────────────────┐
│ 조건                    │ 설명                              │
├─────────────────────────┼───────────────────────────────────┤
│ Production 단계만       │ PoC/MVP에서는 waiver 불허         │
│ ADR 문서 참조 필수      │ 의사결정 기록 추적 가능해야 함     │
│ LLM/Human 검토 필수     │ 자동 승인 불가                    │
│ 본질적 복잡도만         │ 우발적 복잡도는 리팩토링으로 제거  │
└─────────────────────────────────────────────────────────────┘
...
- 규제 준수 로직 (의료, 금융 등)
- 레거시 시스템 통합 (변경 불가)
- 도메인 고유 복잡도 (물리 시뮬레이션 등)
```

#### 3.9.2 의사코드: Waiver 검사
...
FUNCTION check_waiver(
    ...
    """
    Essential Complexity Waiver 검사

    Args:
        source: 분석 대상 소스 코드
        waiver_file: .waiver.json 파일 경로
  ...
  "waivers": [
    {
      ...
      "adr": "ADR-007",
      "justification": "AES-256 암호화 알고리즘 - 본질적 복잡도",
    ...
    },
    {
      ...
      "adr": "ADR-012",
      "justification": "HIPAA 규제 준수 로직 - 법적 요구사항",
